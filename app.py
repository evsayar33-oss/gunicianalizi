...
# ==========================================
st.set_page_config(page_title="TIER-1 MASTER BRAIN (v60.0)", layout="wide", initial_sidebar_state="collapsed")
st.markdown("""
    <style>
...
count = st_autorefresh(interval=60000, limit=None, key="macro_600_refresh")

# ==========================================
# 2. OTONOM ÖĞRENEN QUANT BEYNİ (v60.0)
# ==========================================
class MasterMacroBrain:
    ...
    def calculate_kinetic_series(self, s):
        """15m (%50) + 1H (%30) + 4H (%20) Kinetik İvme Serisi."""
    ...
    def compute_online_learning_ic(self, factor_series, target_price_series, lag=4, window=48):
        ...
        Faktörün son 12 saatteki tahmin doğruluğunu ölçer.
        """
    ...
    def detect_consensus_macro_regime(self, factors):
        ...
        # 1. Geniş Tabanlı Boğa (Teknoloji ve Büyüme katılımı ZORUNLU)
        ...
        # 2. Seçici / Savunmacı Dolar Gevşemesi
        elif dxy < -0.5 and (sector < -0.6 or copper < -0.6):
            return {'name': "⚠️ SEÇİCİ DOLAR GEVŞEMESİ & SAVUNMACI ROTASYON", 'css': "regime-mixed", 'desc': "Dolar düşüşü hisselere taban sağlıyor ancak Teknoloji ve Sanayi zayıf. Ralli seçici."}

        # 3. Reflasyon (Sanayi & Emtia Liderliği)
        elif copper > 0.8 and factors['Gold_Oil'].iloc[-1] > 0 and spx > 0:
            return {'name': "🚀 REFLASYON (Güçlü Büyüme & Sanayi Emtiası Liderliği)", 'css': "regime-reflation", 'desc': "Gümüş ve Bakır küresel büyümeyi fiyatlıyor."}

        # 4. Stagflasyon & Faiz Baskısı
        elif yields > 0.8 and (spx < 0 or nq < 0):
            return {'name': "🌋 STAGFLASYON & LİKİDİTE SIKIŞMASI", 'css': "regime-stagflation", 'desc': "Yükselen faizler hisse değerlemelerini baskılıyor."}

        # 5. Deflasyonist Çöküş
            ...
            return {'name': "⚪ DENGELİ GEÇİŞ REJİMİ (Konsolidasyon)", 'css': "regime-goldilocks", 'desc': "Piyasa ana kırılım öncesinde dengeli konsolide oluyor."}

    def compute_all_asset_scores(self, df):
        scores = {}
        
        # 1. TÜM KİNETİK SERİLER
        ...
        # ==========================================
        # 2. OTONOM ÖĞRENEN & TAVAN KORUMALI MOTOR
        # ==========================================
        def build_master_result(base_weights, target_sym, target_mom_val):
            ...
            for k, w_base in base_weights.items():
                ...
                # Canlı IC / PnL Öğrenme Çarpanı
                learning_factor = np.exp(sign * ic * 0.8)
            ...
            for k, w_base in base_weights.items():
                ...
                # Fiyat İvmesine en az %35 Taban, Dış Rasyolara en fazla %15 Tavan!
                if '_Mom' in k:
        ...
        # ----------------------------------------------------
        # 10 KATMANLI EKSİKSİZ KURUMSAL MATRİSLER (HEPSİ GERİ GELDİ)
        # ----------------------------------------------------
        # 1. GÜMÜŞ (XAG)
        xag_base = {
            'XAG_Mom': 35.0, 'Copper_Gold': 15.0, 'XME_GLD_Ratio': 15.0,
        ...
        scores['XAG'] = build_master_result(xag_base, 'XAG', xag_mom.iloc[-1])
        ...
        scores['XAU'] = build_master_result(xau_base, 'XAU', xau_mom.iloc[-1])
        ...
        scores['SPX'] = build_master_result(spx_base, 'SPX', spx_mom.iloc[-1])
        ...
        scores['NQ'] = build_master_result(nq_base, 'NQ', nq_mom.iloc[-1])
...
# ==========================================
engine = MasterMacroBrain()

st.title("🏛️ TIER-1 MASTER BRAIN (v60.0)")
st.markdown('<span class="status-badge">🧠 AUTONOMOUS 10-LAYER ONLINE LEARNING ENGINE</span>', unsafe_allow_html=True)
st.caption("Eksiksiz 10-Katmanlı Canlı Matris + Tahmin İsabetine Göre Kendi Kendine Öğrenen Zeka")
