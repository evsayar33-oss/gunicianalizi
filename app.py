...
class QuantMacroEngine:
    def __init__(self):
        # yfinance ticker eşleştirmeleri (5 dakikalık barlarda sorun yaratan illikit veriler çıkarıldı/değiştirildi)
        self.tickers = {
            'XAU': 'GC=F', 'XAG': 'SI=F', 'SPX': '^GSPC', 'NQ': 'NQ=F',
            'DXY': 'DX-Y.NYB', 'US10Y': '^TNX', 
            'VIX': '^VIX', 'HYG': 'HYG', 'TLT': 'TLT', 
    ...
    def fetch_market_data(_self):
        """Tüm piyasa verisini asenkron çeker, NaN boşluklarını defansif şekilde doldurur."""
        ...
        # Defansif Veri Doldurma: Önce önceki fiyatla doldur, kalanı sonraki fiyatla doldur
        df = df.ffill().bfill() 
    ...
    def calculate_micro_macro_features(self, df):
        ...
        # 2. VOLATİLİTE (Term Structure & Bond Vol)
        ...
        for col in ['XAU', 'XAG', 'SPX', 'NQ', 'DXY', 'US10Y', 'VIX']:
            # Log getiri hesapla, sonsuz değerleri NaN yap ve 0 ile doldur
    ...
    def dynamic_z_score_normalization(self, df):
        ...
        mean = df.rolling(window=self.z_window, min_periods=10).mean()
        std = df.rolling(window=self.z_window, min_periods=10).std()
        z_scores = (df - mean) / (std + 1e-8) # Sıfıra bölme hatasını önle
        return z_scores.fillna(0) # Z-skor hesaplanamayan yerleri 0 (Nötr) kabul et
    ...
    def calculate_dynamic_weights_and_score(self, z_features, target_ret_col, feature_cols):
        ...
        if len(z_features) < self.z_window:
            return 0.0, {col: 0.0 for col in feature_cols} # Veri yetersizse güvenli çıkış
        ...
        for col in feature_cols:
            ...
            # NaN koruması
            weights[col] = 0 if pd.isna(corr) else corr
...
st.markdown(f"**Gün İçi Likidite ve Çapraz Varlık Akış Motoru v2.6** | Canlı Yenileme: Aktif ({count})")

with st.spinner("Kurumsal Ağlara Bağlanılıyor, Piyasa Verileri Çekiliyor..."):
    try:
        raw_df = engine.fetch_market_data()
        
        if raw_df.empty:
            st.error("Veri seti boş döndü. Piyasa kapalı olabilir veya Yahoo Finance yanıt vermiyor.")
        else:
            ...
            with tab1:
                ...
                with col1:
                    ...
                    st.markdown(f"**Rota:** {'🚀 YUKARI' if score_spx > 10 else '🩸 AŞAĞI' if score_spx < -10 else '⚖️ NÖTR'}")
                ...
                with col2:
                    ...
                    fig.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=220, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='white'))
            ...
            st.markdown("---")
            st.markdown("*v2.6 Notu: Defansif Veri Temizleme (Forward-Fill) ve Hata Yakalama algoritmaları devrede.*")

    except Exception as e:
        st.error(f"Sistem Hatası: {str(e)}\n\nBu durum genellikle yfinance API'sinden gelen anlık bir bağlantı kopmasından kaynaklanır. 60 saniye içinde sistem kendini yenileyecektir.")
